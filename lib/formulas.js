...
// checks if Control Torc power is strong enough for this monster
function canRecruit (character, monster) {
  var worth      = (monster.experience + monster.gold/3).toFixed(2);
  var torcLevels = nconf.get('recruit_levels');
  var power      = null;
  var success    = false;

  _.each(character.inventory, function (item) {
    if (/control torc/i.test(item)) {
      power = Math.max(power, parseInt(item.slice(-1), 10));
    }
  });

  if (monster.recruit && power !== null && !isNaN(worth) && torcLevels[power] > worth) {
    success = true;
  }
...
function recruitment (DQC, character, monster) {
  var A = character.base_strength + character.base_agility + character.base_HP + character.base_MP;
  var B = monster.attack;
  var X = DQC.RNG.integer(0, 31);
  var Y = DQC.RNG.integer(B, 2*B);
  var M = Math.min(A - Y, 16);
  var success = X < M;
