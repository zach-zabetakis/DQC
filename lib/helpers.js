...
function calculateStatBoost (stat_name, base_value, data, character) {
  ...
  _.each(character.equip, function (name, key) {
    ...
    if (stat && item.equip && item.equip[character.job]) {
      addBoost(stat);
  ...
  _.each(character.inventory, function (name) {
    ...
    if (/^E:/.test(name)) {
      ...
      if (stat && accessory.equip && accessory.equip[character.job]) {
        addBoost(stat);
