...
// Take a semicolon separated string of status effects and return an array
function fixStatus (status_string) {
  var statuses = nconf.get('status');
  
  if (status_string) {
    return _.compact(_.map(status_string.split(';'), function (status) {
      status = status.trim();
...
function recalculateStats (data, member, applyEffects) {
  ...
  // max HP
  base_stat = is_monster ? member.max_HP : member.base_HP;
  member.max_HP = calculateStatBoost('HP', base_stat, data, member);
  member.max_HP = Math.max(member.max_HP, 0);

  // max MP
  base_stat = is_monster ? member.max_MP : member.base_MP;
  member.max_MP = calculateStatBoost('MP', base_stat, data, member);
  ...
  // agility
  base_stat = is_monster ? member.agility : member.base_agility;
  member.adj_agility = calculateStatBoost('agility', base_stat, data, member);
  ...
  // miss
  base_stat = is_monster ? member.miss : member.base_miss;
  member.adj_miss = calculateStatBoost('miss', base_stat, data, member);
  member.adj_miss = Math.max(member.adj_miss, 0);

  // critical
  base_stat = is_monster ? member.critical : member.base_critical;
  if (member.job === 'fighter') {
    base_stat += parseInt(member.level / 4, 10);
  }
  member.adj_critical = calculateStatBoost('critical', base_stat, data, member);
  member.adj_critical = Math.max(member.adj_critical, 0);

  // dodge
  base_stat = is_monster ? member.dodge : member.base_dodge;
  if (member.job === 'fighter') {
    base_stat += parseInt(member.adj_agility / 16, 10);
  }
  member.adj_dodge = calculateStatBoost('dodge', base_stat, data, member);
