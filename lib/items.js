...
Item.prototype.useAcornsofLife = function useAcornsofLife(DQC, scenario, member, targets) {
  ...
  if (target.type === 'character') {
    ...
    target.max_HP = helpers.calculateStatBoost('HP', target.base_HP, DQC.data, target);
    target.max_HP = Math.max(target.max_HP, 0);
...
Item.prototype.useEaudeSlime = function useEaudeSlime(DQC, scenario, member, targets) {
  ...
  var slime   = _.cloneDeep(_.find(DQC.data.monster, { name : slimes[index] }));
  var message = '';
  var code    = 65; // symbols start at 'A'

  if (slime) {
    message += 'A horde of ' + slime.name + 's appear!';
    for (var i = 0; i < 8; i++) {
      message += "\n" + slime.name + String.fromCharCode(code + i) + ' attacks!';
      // TODO: insert slime AI here
    }
    message += "\nThe " + slime.name + 's vanish.';
  }
...
Item.prototype.useEaudeUberslime = function useEaudeUberslime(DQC, scenario, member, targets) {
  ...
  if (slime) {
    message += 'A horde of ' + slime.name + 's appears!';
    for (var i = 0; i < 8; i++) {
      message += "\n" + slime.name + String.fromCharCode(code + i) + ' attacks!';
      // TODO: insert slime AI here
    }
...
Item.prototype.useLaStrangaTaso = function useLaStrangaTaso(DQC, scenario, member, targets) {
  ...
  if (options[index] === 'Healing Breath') {
    ...
    targets = this.findTargets(scenario, member);
  }
...
Item.prototype.useMysticNuts = function useMysticNuts(DQC, scenario, member, targets) {
  ...
  if (target.type === 'character') {
    ...
    target.max_MP = helpers.calculateStatBoost('MP', target.base_MP, DQC.data, target);
    target.max_MP = Math.max(target.max_MP, 0);
