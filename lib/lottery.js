  ...
  _.each(DQC.data.character, function (character) {
    _.each(character.bol, function (ticket) {
      ...
      if (matches === 4 && bol === number) {
        ...
        messages.push('WINNER! ' + character.displayName() + ': Jackpot! +' + DQC.scenario.jackpot + ' gold! (' + ticket + ')');
        character.gold += DQC.scenario.jackpot;
        DQC.scenario.jackpot = 50000;
      } else if (matches === 4) {
        // first prize -- 5000G
        messages.push('WINNER! ' + character.displayName() + ': First Prize! +5000 gold! (' + ticket + ')');
        character.gold += 5000;
      } else if (matches === 3 && bol === number) {
        // second prize -- 2500G
        messages.push('WINNER! ' + character.displayName() + ': Second Prize! +2500 gold! (' + ticket + ')');
        character.gold += 2500;
      } else if (matches === 3 || (matches === 2 && bol === number)) {
        // third prize -- 500G
        messages.push('WINNER! ' + character.displayName() + ': Third Prize! +500 gold! (' + ticket + ')');
        character.gold += 500;
      } else if (matches === 2 || (matches === 1 && bol === number)) {
        // fourth prize -- 100G
        messages.push('WINNER! ' + character.displayName() + ': Fourth Prize! +100 gold! (' + ticket + ')');
        character.gold += 100;
      } else if (bol === number) {
        // fifth prize -- 50G
        messages.push('WINNER! ' + character.displayName() + ': Fifth Prize! +50 gold! (' + ticket + ')');
        character.gold += 50;
      }
    });

    if (character.bol.length) {
      expires.push(character.displayName());
      character.bol = [];
    }
  });
...
function loto3 (DQC) {
  ...
  _.each(DQC.data.character, function (character) {
    _.each(character.loto3, function (ticket) {
      ...
      if (matches === 3) {
        // grand prize -- 500G
        messages.push('WINNER! ' + character.displayName() + ': First Prize! +500 gold! (' + ticket + ')');
        character.gold += 500;
      } else if (matches === 2) {
        // second prize -- 40G
        messages.push('WINNER! ' + character.displayName() + ': Second Prize! +40 gold! (' + ticket + ')');
        character.gold += 40;
      }
    });
...
// Generate a random Ball of Light ticket
function randomBoL (RNG) {
  var ticket = [];
  var number;

  do {
    number = RNG.integer(1, 25);
    if (!_.contains(ticket, number)) {
      ticket.push(number);
    }
  } while (ticket.length < 4);

  ticket = ticket.join(',') + '/' + RNG.integer(1, 16);

  return ticket;
}

// Generate a random Loto 3 ticket
function randomLoto3 (RNG) {
  var ticket = RNG.integer(0, 999) + '';
  while (ticket.length < 3) { ticket = '0' + ticket }
  return ticket;
}
