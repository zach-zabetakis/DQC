...
module.exports = function (battleHelpers) {
  ...
  function attack (DQC, scenario, command) {
    ...
    if (target.is_dead && member.is_aware !== false) {
      command.target = target = battleHelpers.retarget(scenario, target);
    }

    var prefix = (command.flavor_prefix || '').trim();
    ...
    var suffix = (command.flavor_suffix || '').trim();
  ...
  function charge (DQC, scenario, command) {
    var member     = command.member;
    var dispName   = member.displayName();
    ...
    if (group) {
      ...
      target_index = _.findIndex(group.members, { name : command.target.name });
  ...
  function dismiss (DQC, scenario, command) {
    var member   = command.member;
    var dispName = member.displayName();
    var name     = command.name;
    var message  = dispName + ' dismisses ' + name + ' from battle!';
    var recruit  = _.find(scenario.allies, { name : name, owner : member.name });
    var index    = _.findIndex(DQC.data.recruit, { name : name, owner : member.name });

    if (recruit) {
      battleHelpers.expelFromBattle(scenario, recruit);
      if (index > -1) {
        DQC.data.recruit[index] = recruit;
      }
    }
  ...
  // Do nothing
  function none (DQC, scenario, command) {
    var member   = command.member;
    var dispName = member.displayName();
  ...
  function parry (DQC, scenario, command) {
    var member   = command.member;
    var dispName = member.displayName();
  ...
  function recall (DQC, scenario, command) {
    var member       = command.member;
    var dispName     = member.displayName();
    var name         = command.name;
    var message      = dispName + ' calls ' + name + ' into battle!';
    var recruit      = _.find(DQC.data.recruit, { name : name, owner : member.name });
    var position     = (command.extra === 'after') ? 1 : 0;
    var target       = command.target || {};
    var target_group = _.findValue(scenario.battle.allies, 'groups.' + target.group_index);
    var target_index;

    if (recruit) {
      // recruits parry to make up for the lack of action upon joining the battle
      recruit.parry = true;
      recruit = battleHelpers.joinBattle(DQC, scenario, recruit);
  ...
  // Coupled with PARRY command to halve incoming damage for the turn.
  function retreat (DQC, scenario, command) {
    var member     = command.member;
    var dispName   = member.displayName();
  ...
  // Run away
  function run (DQC, scenario, command) {
    var member     = command.member;
    var dispName   = member.displayName();
  ...
  // Coupled with PARRY command to halve incoming damage for the turn.
  function shift (DQC, scenario, command) {
    var member       = command.member;
    var dispName     = member.displayName();
    ...
    var position     = (command.name === 'after') ? 1 : 0;
    var member_index;
    var target_index;
    var new_group;

    if (command.name === 'new' && member_group) {
      ...
      if (member_index !== -1) {
        ...
        new_group.front = scenario.battle.has_fronts ? command.extra : null;
