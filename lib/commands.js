...
var Spells   = require(__dirname + '/spells');
...
module.exports = function (battleHelpers) {
  ...
  function spell (DQC, scenario, member) {
    var spells     = new Spells(DQC.data.spell);
    var spell      = spells.getSpell(member.command.name);
    var dispName   = member.displayName();
    var target     = member.command.target;
    var is_bounced = _.includes(target.status, 'BO');
    var message    = '';
  ...
  function validateSpell (data, command) {
    ...
    var spell      = new Spells(data.spell).getSpell(command.name);
