...
module.exports = function (battleHelpers) {
  ...
  function recall (DQC, scenario, member) {
    var dispName = member.displayName();
    var target   = member.command.name;
    var message  = dispName + ' calls ' + target + ' into battle!';
    var recruit  = _.find(DQC.data.recruit, { name : target, owner : member.name });
    var monster;

    if (recruit) {
      monster = _.find(DQC.data.monster, { name : recruit.species });
      if (monster) {
        recruit = _.merge({}, monster, recruit);
        // TODO: fix status and effects
