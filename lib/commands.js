    ...
    if (has_target) {
      _.each(data.scenario.scenarios, function (scenario) {
        ...
        if (found !== -1) {
          ...
          // if target was found make sure member is in the same scenario
          group = scenario.battle[member_group].groups[command.member.group_index];
          found = group && _.findIndex(group.members, { name : command.member.name });
          if (found === -1) {
            throw new Error(invalidMsg + 'could not be found in the same scenario as the target.');
          }
        }
      });
    }
