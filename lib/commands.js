...
module.exports = function (battleHelpers) {
  ...
  function attack (DQC, scenario, member) {
    ...
    function attackAllTargets (target) {
      ...
      if (target.in_battle && !target.is_dead) {
        count++;
        if (results.is_crit) {
        ...
        if (results.is_miss) {
          ...
          _.each(results.onHits, function (onHit) {
            ...
            if (onHit.status) {
              onHit.message = battleHelpers.applyStatus(onHit.status, target);
            }
            message += ' ' + onHit.message;
          });

          // apply damage to target
          battleHelpers.takeDamage(scenario, target, results.damage);
          if (target.is_dead) {
  ...
  function skill (DQC, scenario, member) {
    ...
    if (skill.has_skill) {
      ...
      if (DQC.RNG.bool(skill.miss, 32)) {
        ...
        // use the skill
        message += ' ' + skill.useSkill(scenario, member, targets, DQC.RNG);
      }
    }
  ...
  function spell (DQC, scenario, member) {
    ...
    if (spell.has_spell) {
      message = helpers.format('"' + spell.invocation + '!"', false, true) + ' ' + dispName + ' casts ' + spell.name + '!';
      if (member.curr_MP < spell.MP) {
        ...
        // cast the spell
        message += ' ' + spell.castSpell(scenario, member, targets, DQC.RNG);
      }
