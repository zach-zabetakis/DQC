...
module.exports = function (battleHelpers) {
  ...
  function retreat (DQC, scenario, member) {
    ...
    if (group) {
      ...
      target_index = _.findIndex(group.members, { name : member.command.target.name });
      if (member_index !== -1 && target_index !== -1) {
        console.log(member_index, target_index);
  ...
  function validateAttack (command) {
    // no additional validation necessary
    return true;
  }

  // Validate a CHARGE command
  function validateCharge (command) {
  ...
  function validateHeart (command) {

  }

  // Validate an ITEM command
  function validateItem (command) {

  }

  // Validate a NONE command
  function validateNone (command) {
    // good job!
    return true;
  }

  // Validate a PARRY command
  function validateParry (command) {
    // no additional validation necessary
    return true;
  }

  // Validate a RETREAT command
  function validateRetreat (command) {
  ...
  function validateRun (command) {
    // no additional validation necessary
    return true;
  }

  // Validate a SHIFT command
  function validateShift (command) {

  }

  // Validate a SKILL command
  function validateSkill (command) {

  }

  // Validate a SPELL command
  function validateSpell (command) {
