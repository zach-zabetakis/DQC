...
module.exports = function (battleHelpers) {
  ...
  function heart (DQC, scenario, member) {
    ...
    if (ability) {
      message += dispName + "'s heart glows! ";
      // heart abilities can mimic spells, skills, or items
      switch (ability.type) {
        case 'ITEM':
          // TODO: mimic an item's effect
          break;
        case 'SKILL':
          var skill = new Skill(ability.name, DQC.data.skill);
          targets = skill.findTargets(scenario, member);
          message += skill.displayMessage(dispName) + ' ' + skill.useSkill(DQC, scenario, member, targets);
          break;
        case 'SPELL':
          var spell = new Spell(ability.name, DQC.data.spell);
          targets = spell.findTargets(scenario, member);
          message += spell.displayMessage(dispName) + ' ' + spell.castSpell(DQC, scenario, member, targets);
          break;
        default:
          break;
      }
  ...
  function skill (DQC, scenario, member) {
    var skill    = new Skill(member.command.name, DQC.data.skill);
    var dispName = member.displayName();
    var targets  = [];
    var message  = '';
  ...
  function spell (DQC, scenario, member) {
    var spell    = new Spell(member.command.name, DQC.data.spell);
    var dispName = member.displayName();
    var targets  = [];
    var message  = '';
    var useMP    = true;
  ...
  function validateHeart (data, command) {
    ...
    if (!command.name) {
      ...
      throw new Error(invalidMsg + 'heart data not found for member ' + command.member.displayName());
    ...
    if (!ability) {
      ...
      switch (ability.type) {
        ...
        case 'SKILL':
          var skill = new Skill();
          if (!skill.findSkill(ability.name, data.skill)) {
            throw new Error(invalidMsg + 'skill name ' + ability.name + ' not found.');
          }
          break;
        case 'SPELL':
          var spell = new Spell();
          if (!spell.findSpell(ability.name, data.spell)) {
  ...
  function validateSkill (data, command) {
    ...
    var skill      = new Skill(command.name, data.skill);

    if (!command.name) {
      throw new Error(invalidMsg + 'command must include a skill name.');
    } else if (!skill.has_skill) {
  ...
  function validateSpell (data, command) {
    ...
    var spell      = new Spell(command.name, data.spell);

    if (!command.name) {
      throw new Error(invalidMsg + 'command must include a spell name.');
    } else if (!spell.has_spell) {
