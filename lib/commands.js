...
module.exports = function (battleHelpers) {
  ...
  function item (DQC, scenario, member) {
    var dispName = member.displayName();
    var message  = dispName + ' is using the ' + member.command.name + '!';
  ...
  function validateItem (data, command) {
    ...
    var item       = helpers.findItem(data, command.name);
    var ability;

    if (!command.name) {
      throw new Error(invalidMsg + 'command must include an item name.');
    } else if (!battleHelpers.hasItem(command.member, command.name)) {
      throw new Error(invalidMsg + 'inventory or equipment does not contain this item.');
    } else if (!item) {
