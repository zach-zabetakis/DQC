...
internals.Action.prototype.targetingLogicHealing = function targetingLogicHealing (DQC, scenario, member) {
  var targetSelf = (this.target === 'single') && DQC.RNG.bool(3, 4);
  ...
  if (targetSelf) {
    if ((this.minimum || this.range) && testHPthreshold(member)) {
      target = member;
    } else if (status && _.includes(member.status, status)) {
      target = member;
    }
  }
    ...
    if (memberPool.length) {
      ...
      if (this.minimum || this.range) {
        ...
        memberPool = _.filter(memberPool, function (candidate) {
          return _.includes(candidate.status, status);
        });
...
internals.Action.prototype.targetingLogicPhysical = function targetingLogicPhysical (DQC, scenario, member) {
  ...
  if (member.is_enemy) {
    ...
    target = AI.findWeakestRemaining(scenario, 'characters', null, member.front, true) || {};
  }
