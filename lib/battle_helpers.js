  ...
  _.each(scenario.battle[group_type].groups, function (group, group_index) {
    ...
    if (index > -1) {
      ...
      if (!group.members.length) {
        ...
        scenario.battle[group_type].groups.active = isActive(group.members);
      }
...
function victory (DQC, scenario) {
  ...
  _.each(scenario.battle.enemies.groups, function (group) {
    _.each(group.members, function (enemy) {
      ...
      goldTotal += enemy.gold - parseInt((enemy.gold * DQC.RNG.integer(1, 256)) / 1024, 10);

      var drop = enemy.drop;
      if (enemy.killed_by) {
        // Check for treasures (priority: common > rare > heart)
        // TODO: place in inventory unless full
        if (drop.common.name && DQC.RNG.bool(enemy.drop.common.rate, 256)) {
          treasures.push('Fortune smiles upon thee, ' + enemy.killed_by + ', for thou hast obtained the ' + drop.common.name + '.');

        } else if (drop.rare.name && DQC.RNG.bool(enemy.drop.rare.rate, 256)) {
          treasures.push('Fortune smiles upon thee, ' + enemy.killed_by + ', for thou hast obtained the ' + drop.common.name + '.');

        } else if (drop.heart.name && DQC.RNG.bool(enemy.drop.heart.rate, 1024)) {
          var heartDesc = _.result(_.findWhere(DQC.data.heart, { name : drop.heart.name }), 'description');
          treasures.push('Fortune smiles upon thee, ' + enemy.killed_by + ', for thou hast obtained the ' + drop.heart.name + ' heart.');
          treasures.push(heartDesc);
        }
      }
    });
  });
  ...
  _.each(scenario.characters, function (character) {
    ...
    //TODO: updateExperience(character, xpTotal);
  });
