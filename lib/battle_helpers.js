var AI               = require(__dirname + '/ai_helpers');
...
function simulateMonsterTurn (DQC, scenario, monster) {
  ...
  if (hasTurn) {
    if (!monster.command) {
      ...
      AI.chooseCommand(DQC, scenario, monster, monster.front);
    }
...
function simulateNPCTurn (DQC, scenario, npc) {
  ...
  if (hasTurn) {
    if (!npc.command) {
      ...
      AI.chooseCommand(DQC, scenario, npc, npc.front);
    }

    // perform the command
    message += performCommand(DQC, scenario, npc);
  }
