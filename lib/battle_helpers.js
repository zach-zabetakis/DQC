...
function healDamage (member, amount) {
  if (!member.is_dead) {
...
function takeDamage (member, amount) {
  if (!member.is_dead) {
    member.curr_HP -= amount;
    checkHP(member);
  }
