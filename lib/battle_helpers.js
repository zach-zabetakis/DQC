...
function simulateMonsterTurn (DQC, scenario, monster) {
  ...
  if (!canAct(monster)) {
    if (_.includes(monster.status, 'FR')) {
      // FEAR only lasts a single turn
      monster.status = _.difference(monster.status, ['FR']);
      DQC.out(disp_name + ' is frozen in fear!');

    } else if (_.includes(monster.status, 'SL')) {
      // SLEEP has a 1/2 chance of being cured
      if (DQC.RNG.integer(0, 255) % 2 === 0) {
        monster.status = _.difference(monster.status, ['SL']);
        DQC.out(disp_name + ' wakes up!');
      } else {
        DQC.out(disp_name + ' is asleep.');
      }

    } else if (_.includes(monster.status, 'NU')) {
      // NUMB has a 1/8 chance of being cured
      if (DQC.RNG.integer(0, 255) % 8 === 0) {
        monster.status = _.difference(monster.status, ['NU']);
        DQC.out(disp_name + ' is cured of paralysis!');
      } else {
        DQC.out(disp_name + ' is paralyzed.');
      }
    }

    // exit out of the monster turn
    return;
  }
  ...
  switch (monster.behavior) {
    case 'fixed':
      ...
      index  = (scenario.battle_turn % monster.pattern.length) || 0;
  ...
  DQC.out(disp_name + ' ' + action + 's ' + target.name + '!');
}
