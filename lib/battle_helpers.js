...
function applyAttackResults (results, scenario, member, target, verbose) {
  var message = '';
  verbose = !!verbose;
  ...
  if (isTargetable(target)) {
    ...
    if (results.is_miss) {
      ...
      _.each(results.onHits, function (onHit) {
        if (results.damage) {
          results.damage += target.parry ? parseInt(onHit.damage / 2, 10) : onHit.damage;
        }
        message += ' ' + onHit.message;
        if (onHit.status) {
          message += ' ' + applyStatus(onHit.status, target);
        }
      });
    ...
    if (!_.includes(member.status, status) && !_.includes(['ALL', 'NEG', 'POS'], status)) {
      ...
      switch (status) {
        ...
        case 'NU':
          ...
          break;
        case 'POS':
          member.status = _.difference(member.status, ['BA' , 'BO', 'DR', 'IR']);
...
function joinBattle (DQC, scenario, member, type, is_enemy) {
  member.type = type;
  member.is_enemy = is_enemy;
  member.in_battle = true;
  member.can_act = true;
  member.can_target = true;
...
function performCommand (DQC, scenario, member) {
  ...
  if (typeof Commands[type] === 'function') {
    message = Commands[type](DQC, scenario, member);
  } else {
...
function retarget (scenario, target) {
  ...
  var group_type   = battleHelpers.groupType(target);
  ...
  if (target_group && target_group.active) {
    sorted_group = _.filter(_.sortBy(target_group.members, function (member) {
      return formulas.survival(member, true);
    }), isTargetable);

    if (sorted_group.length) {
      weakestRemaining = sorted_group.slice(-1)[0];
      weakestActive    = _.filter(sorted_group, function (member) { return !isIncapacitated(member); }).pop();
      retarget = weakestActive || weakestRemaining || target;
    }
  }
...
function simulateMonsterTurn (DQC, scenario, monster) {
  ...
  if (hasTurn) {
    if (monster.command) {
      // perform the command
      message += performCommand(DQC, scenario, monster);
