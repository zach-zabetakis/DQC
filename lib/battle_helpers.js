...
function deathPenalty (DQC, character) {
  ...
  var xpPenalty = parseInt(xpTable[character.level - 1] - xpTable[character.level], 10);
console.log(character.level);console.log(xpPenalty);
  character.experience -= xpPenalty;
...
function simulateMonsterTurn (DQC, scenario, monster) {
  ...
  if (isIncapacitated(monster)) {
    checkStatusRecovery(DQC, scenario, monster);
...
function wipeout (DQC, scenario) {
  // each character in the party suffers XP/gold penalty
  _.each(scenario.characters, function (character) {
    deathPenalty(DQC, character);
  });
