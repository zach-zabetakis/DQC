...
function calculateStatGrowths (DQC, character) {
  ...
  var mod        = build % 4;
  var bonus      = (nconf.get('stat_bonus') === true);
  ...
  if (bonus) {
    growths.STR += DQC.RNG.bool(growths.STR || 1, 16) ? 1 : 0;
  }
  ...
  if (bonus) {
    growths.AGI += DQC.RNG.bool(growths.AGI || 1, 16) ? 1 : 0;
  }
  ...
  if (bonus) {
    growths.HP += DQC.RNG.bool(growths.HP || 1, 16) ? 1 : 0;
  }
  ...
  if (bonus) {
    growths.MP += DQC.RNG.bool(growths.MP || 1, 16) ? 1 : 0;
  }
  character.base_MP += growths.MP || 0;

  // recalculate stats
  helpers.recalculateStats(DQC.data, character, true);
