...
// determines if a member is able to cast the currently set spell
Spell.prototype.canCast = function canCast(member) {
  var can_cast = true;

  if (member.can_cast === false) {
    if (_.includes(member.status, 'ST') || member.curr_MP < this.MP) {
      can_cast = false;
    }
  }
...
Spell.prototype.castOutside = function castOutside(DQC, scenario, member, targets) {
  ...
  var message    = '';
  var mp_cost    = this.MP;
  var max        = parseInt(member.curr_MP / mp_cost) - 1;
  var target;
  var index;
  ...
  if (max >= 1) {
    ...
    // prompt for additional targets
    prompt('Choose up to ' + max + ' additional targets (comma separated): ', function (input) {
      input = input.split(',');
      _.each(input, function (name) {
        name = name.trim();

        index = _.findIndex(memberPool, function (m) { return m.displayName() === name });
        if (index > -1) {
          targets.push(memberPool[index]);
          this.MP += mp_cost;
        }
      });
  ...
  for (var i = 0; i < targets.length; i++) {
    ...
    if (target.in_battle) {
      if (scenario.is_indoors) {
        ...
        if (target.is_dead) {
          message += battleHelpers.defeated(DQC, target);
        }
...
// custom spell routine for Sacrifice spell
Spell.prototype.castSacrifice = function castSacrifice(DQC, scenario, member, targets) {
  var messages = [];
  var message  = '';
  var target;
  var resist;
  ...
  for (var i = 0; i < targets.length; i++) {
    ...
    if (battleHelpers.isTargetable(target)) {
      ...
      resist = target.resist['beat'] || 0;

      // for Sacrifice only, SAVER check comes first and mitigates all damage
      if (target.saver['phys'] && DQC.RNG.bool(1, 4)) {
        message += 'Saved!';
      } else {
        if (!DQC.RNG.bool(resist, 16)) {
          ...
          // resisting Sacrifice reduces HP to 1
          amount = Math.max(target.curr_HP - 1, 0);
  ...
  // Sacrifice takes the life of the caster
  if (!member.is_dead) {
    battleHelpers.applyStatus('DE', member);
    message += "\n" + member.displayName() + ' collapses from exhaustion.';
    message += battleHelpers.defeated(DQC, member);
    battleHelpers.updateActiveGroups(scenario, battleHelpers.groupType(member));
  }
...
Spell.prototype.castSleepmore = function castSleepmore(DQC, scenario, member, targets) {
  ...
  if (battleHelpers.isTargetable(target)) {
    ...
    resist = target.resist['sleep'] >= 16 ? 16 : (target.resist['sleep'] === 15) ? 8 : 0;
...
Spell.prototype.castSpell = function castSpell(DQC, scenario, member, targets) {
  ...
  var overrideFunction = 'cast' + helpers.toFunctionName(this.name);
