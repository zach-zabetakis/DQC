...
internals.Spell.prototype.castOutside = function castOutside(DQC, scenario, member, targets, is_spell) {
  var messages = [];
  var message  = '';
  var target;
...
internals.Spell.prototype.castRadiant = function castRadiant(DQC, scenario, member, targets, is_spell) {
  var message = '';
...
internals.Spell.prototype.castRepel = function castRepel(DQC, scenario, member, targets, is_spell) {
  var messages = [];
  var message  = '';
  var target;
  ...
  for (var i = 0; i < targets.length; i++) {
    ...
    if (battleHelpers.isTargetable(target)) {
      if (is_spell && _.includes(target.status, 'BO') && member !== target) {
...
internals.Spell.prototype.castReturn = function castReturn(DQC, scenario, member, targets, is_spell) {
  var messages = [];
  var message  = '';
  var target
...
internals.Spell.prototype.castSpell = function castSpell(DQC, scenario, member, targets, is_spell) {
  var max_stat  = nconf.get('max_stat');
  ...
  // distinguish spells from abilities that mimic a spell's effect
  is_spell = (is_spell !== false);
  ...
  if (typeof this[overrideFunction] === 'function') {
    message = this[overrideFunction](DQC, scenario, member, targets, is_spell);
    return message;
  }
    ...
    if (battleHelpers.isTargetable(target) || (target.is_dead && this.type === 'revival')) {
      ...
      switch (this.type) {
        ...
        case 'buff':
          ...
          if (_.includes(['Bikill'], this.name) && _includes(member.effects, this.name)) {
            message += 'no effect.';
