      ...
      switch (this.type) {
        ...
        case 'buff':
          if (this.stat_from && this.stat_to) {
            amount = target[this.stat_to];
            amount += parseInt(target[this.stat_from] * this.multiplier, 10);
            if (max_stat[this.stat_to]) {
              amount = Math.min(amount, max_stat[this.stat_to]);
            }
            target[this.stat_to] = amount;

            // TODO: amount should be only the value that has gone up
