...
internals.Spell.prototype.castOutside = function castOutside(scenario, member, target, RNG) {
  var message = '';

  if (scenario.is_indoors) {
    message = 'Transported outside.';
    battleHelpers.expelFromBattle(scenario, target);
    // TODO: Additional messaging/scenario for those who have departed?
  } else {
    message = 'no effect.';
  }
...
internals.Spell.prototype.castRadiant = function castRadiant(scenario, member, target, RNG) {
  var message = '';
...
internals.Spell.prototype.castRepel = function castRepel(scenario, member, target, RNG) {
  var message = '';
...
internals.Spell.prototype.castReturn = function castReturn(scenario, member, target, RNG) {
  var message = '';
...
internals.Spell.prototype.castSpell = function castSpell(scenario, member, targets, RNG) {
  ...
  for (var i = 0; i < targets.length; i++) {
    ...
    if (target.in_battle && (!target.is_dead || this.type === 'revival')) {
      ...
      switch (this.type) {
        ...
        case 'buff':
          if (this.stat_from && this.stat_to) {
            ...
            // reduce the amount by the range until the stat cap
            if (max_stat[this.stat_to]) {
        ...
        case 'debuff':
          ...
          if (!RNG.bool(resist, 16)) {
            if (saver && target.saver[saver] && RNG.bool(1, 4)) {
              ...
              if (this.stat_from && this.stat_to) {
                ...
                // reduce the amount by the range until the stat hits 0
          ...
          break;
        case 'travel':
          // travel spells all have custom behavior
          if (this.name === 'Outside') {
            message += this.castOutside(scenario, member, target, RNG);
          } else if (this.name === 'Radiant') {
            message += this.castRadiant(scenario, member, target, RNG);
          } else if (this.name === 'Repel') {
            message += this.castRepel(scenario, member, target, RNG);
          } else if (this.name === 'Return') {
            message += this.castReturn(scenario, member, target, RNG);
          }
          break;
        default:
