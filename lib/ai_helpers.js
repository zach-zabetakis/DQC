...
module.exports = function (battleHelpers) {
  ...
  function findWeakestRemaining (scenario, group_type, group_index, front, filter_active) {
    ...
    // find valid list of targetable groups, optionally filtered by front and/or a specific group index
    if (target_groups.length) {
      if (scenario.battle.has_fronts && front && front !== 'ALL') {
        if (typeof group_index === 'number') {
          target_groups = _.filter(target_groups, function (group, index) {
            return (group.active && (group.front === front || group.front === 'ALL') && (index === group_index));
          });
        } else {
          target_groups = _.filter(target_groups, function (group) {
            return (group.active && (index === group_index));
          });
        }
      } else {
        if (typeof group_index === 'number') {
    ...
    if (target_groups.length) {
      ...
      _.each(target_groups, function (group) {
        target_members.push(_.filter(group.members, battleHelpers.isTargetable));
      });
      // sort by survivability score
      target_members = _.sortBy(_.flatten(target_members), function (member) {
